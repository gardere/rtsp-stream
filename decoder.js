...
Decoder.prototype._writeOffset = function (chunk, offset, cb) {
  while (offset < chunk.length) {
    ...
    if (this._inBody) {
      ...
      if (offset === 0) {
        // chunk not consumed - _writeOffset will be called again when ready
        debug('not ready to consume chunk');
        return;
      }
...
Decoder.prototype._writeHead = function (chunk, offset) {
  ...
  if (
    ~NO_BODY_STATUS_CODES.indexOf(this._msg.statusCode)
    || this._msg.method === 'SET_PARAMETER' //to handle incorrectly formed message from hub
    ) {
    this._bodySize = 0
  } else {
